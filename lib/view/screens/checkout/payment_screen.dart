...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  void initState() {
    super.initState();
    selectedUrl = '${AppConstants.BASE_URL}/payment-mobile?token=${widget.url}&&payment_method=${Provider.of<OrderProvider>(context, listen: false).paymentMethod}';
    _initData();
  }
...
class MyInAppBrowser extends InAppBrowser {
  ...
  void _pageRedirect(String url) {
    if(_canRedirect) {
      bool _isUrl = url.contains('${AppConstants.BASE_URL}${RouteHelper.orderSuccessful}');
      bool _isSuccess = url.contains('success?') && _isUrl;
      bool _isFailed = url.contains('fail?') && _isUrl;
      bool _isCancel = url.contains('cancel?') && _isUrl;
      print('----------------payment status -----$_isCancel -- $_isSuccess -- $_isFailed');
      print('------------------url --- $url');
